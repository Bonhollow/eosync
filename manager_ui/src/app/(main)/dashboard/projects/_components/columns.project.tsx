    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <form onSubmit={handleSave} className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="budget_total" className="text-right">
              Budget
            </Label>
            <Input
              id="budget_total"
...
const AssignmentsModal = ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Failed to save assignment. The employee might already be assigned to this task.",
      );
    }
  };
  ...
  const availableEmployees = employees.filter(
    (emp) => !task.assignments.some((a) => a.employee_id === emp.id),
  );
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        {isFormOpen && (
          <form
            onSubmit={handleSave}
            className="p-4 mb-6 border rounded-lg bg-muted/50"
          >
            ...
            <div className="grid gap-4">
              ...
              <Input
                id="role_on_task"
                placeholder="Role on task"
                value={formData.role_on_task || ""}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    role_on_task: e.target.value,
                  }))
                }
              />
              <Input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    assigned_hours: Number(e.target.value) || undefined,
                  }))
                }
              />
            </div>
            ...
            >
              <div>
                <p className="font-semibold">
                  #{a.employee.id} {a.employee.last_name} ({a.employee.role})
                </p>
                <p className="text-sm text-muted-foreground">
                  {a.role_on_task} - {a.assigned_hours} hours
                </p>
              </div>
              <Button
                variant="ghost"
                ...
                onClick={() => handleDelete(a.employee_id, a.task_id)}
              >
                <Trash className="size-4" />
              </Button>
            </div>
          ))}
          {task.assignments.length === 0 && !isFormOpen && (
...
export const getProjectsColumns = (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const [selectedTaskForAssignments, setSelectedTaskForAssignments] =
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex justify-end mb-4">
                {!isFormOpen && (
                  <div className="flex items-center gap-2">
                    <Button size="sm" onClick={handleAutomateAssignments}>
                      <Bot className="mr-2 size-4" /> Automate
                    </Button>
                    <Button
                      size="sm"
                      onClick={() => {
              ...
              <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <Button
                        ...
                        size="sm"
                        className="flex items-center gap-2"
                        onClick={() => setSelectedTaskForAssignments(task)}
                      >
                ...
                {project.tasks.length === 0 && !isFormOpen && (
                  <p className="text-center text-muted-foreground py-4">
                    No tasks found.
                  </p>
                )}
    ...
    cell: ({ row }) =>
      row.getValue("budget_total")
        ? new Intl.NumberFormat("en-US", {
            style: "currency",
            currency: "USD",
          }).format(parseFloat(row.getValue("budget_total")))
        : "-",
  },
  {
    id: "actions",
