...
export function EmployeeAssignmentsCard({
  ...
  const currentMonthYear = format(
    new Date(scheduleData.start_of_week),
    "MMMM yyyy",
  );
  ...
  return (
    <Card className={isLoading ? "opacity-50 transition-opacity" : ""}>
      <CardHeader>
        ...
        <CardDescription>
          View resource allocation by project or by weekly schedule.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="overflow-x-auto border rounded-lg">
          ...
          >
            <div className="sticky left-0 bg-muted font-semibold text-sm p-2 border-b border-r">
              Employee
            </div>
            {weekDates.map((date) => (
              <div
                key={date.toISOString()}
                className="font-semibold text-sm p-2 text-center border-b"
              >
                {format(date, "eee dd")}
              </div>
            ))}
              ...
              <div key={employee.employee_id} className="contents">
                ...
                {weekDates.map((date) => {
                  ...
                  return (
                    <div
                      key={dateStr}
                      className="p-1 border-b space-y-1 min-h-[60px]"
                    >
                      {tasksForDay.map((task) => (
                        <div
                          key={task.task_id}
                          title={`${task.task_title} (${task.project_title})`}
