...
export const leaveCreateSchema = z.object({
  employee_id: z.number({ required_error: "Employee is required." }).min(1, "Employee is required."),
  type: leaveTypeEnum,
  approved: z.boolean().optional(),
  start_date: z.string({ required_error: "Start date is required." }).min(1, "Start date cannot be empty."),
  end_date: z.string({ required_error: "End date is required." }).min(1, "End date cannot be empty."),
  reason: z.string().nullish(),
});
