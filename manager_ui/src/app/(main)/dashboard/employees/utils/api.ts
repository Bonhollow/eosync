...
export async function createEmployee(emp: object | object[]) {
  const response = await axios.post("http://localhost:8000/employees", emp);
  return response.data;
}

export async function editEmployee(id: number, emp: object | object[]) {
...
export async function createEmployeesFromFile(file: File) {
  ...
  const response = await axios.post(
    "http://localhost:8000/upload/file/elaborate_employees",
    formData,
    {
      headers: {
